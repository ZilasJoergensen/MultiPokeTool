...
type ViewMode = 'stored' | 'living-dex';
...
export function StoragePage() {
  const [storage, refetch] = useStoreValue(listStorage, ['storage']);
  ...
  const [view, setView] = useState<ViewMode>('stored');
  ...
  const ownedIds = useMemo(() => new Set((storage ?? []).map((c) => c.pokemonId)), [storage]);
  const shinyIds = useMemo(
    () => new Set((storage ?? []).filter((c) => c.shiny).map((c) => c.pokemonId)),
    [storage],
  );
  ...
  const filtered = useMemo(() => {
    ...
    return items;
  }, [storage, shinyOnly, gameFilter, searchQ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        {(['stored', 'living-dex'] as const).map((v) => (
          <button
            key={v}
            type="button"
            onClick={() => setView(v)}
          ...
          >
            {v === 'stored' ? 'Stored Pokémon' : 'Living Dex'}
          </button>
        ))}
      </div>

      {/* Stored Pokémon tab description */}
      {view === 'stored' && (
        <p className="text-sm text-muted">Individual Pokémon you currently own across your games.</p>
      )}

      {/* Living Dex tab description */}
      {view === 'living-dex' && (
        <p className="text-sm text-muted">Own one of every Pokémon at the same time. Slots are filled only by Pokémon currently in Storage.</p>
      )}

      {/* Filters (Stored Pokémon only) */}
      {view === 'stored' && (
        ...
        <>
          {filtered.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
              {filtered.map((entry) => (
                <StorageCard key={entry.id} entry={entry} onClick={() => setEditTarget(entry)} />
              ))}
            </div>
          )}
        </>
...
function StorageCard({ entry, onClick }: { entry: StoredPokemon; onClick: () => void }) {
...
function EntryModal({
  ...
  function handleDelete() {
    if (confirm(`Remove ${picked ? prettyName(picked.name) : 'this Pokémon'} from your storage?`)) {
      onDelete?.();
    }
  }
      ...
      >
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold">
            {mode === 'add' ? 'Add to Storage' : 'Edit Pokémon'}
          </h2>
        ...
        <div className="flex gap-3 pt-1">
          ...
          <button type="submit" disabled={!picked} className="btn btn-primary flex-1 disabled:opacity-40">
            {mode === 'add' ? 'Add to Storage' : 'Save Changes'}
          </button>
        </div>
      </form>
    </div>
